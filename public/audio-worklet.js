...
class DISStreamProcessor extends AudioWorkletProcessor {
  constructor() {
    ...
    this._wp = 0;
    this._rp = 0;
    this._PREROLL = Math.round(sampleRate * 0.2); // 200ms before first output
  ...
  process(inputs, outputs) {
    ...
    if (!this._started) {
      if (this._avail() >= this._PREROLL) this._started = true;
      else { out.fill(0); return true; }
    }
    const avail = this._avail();
    if (avail < out.length) {
      // Underrun: reset and wait for next preroll
      out.fill(0);
      this._started = false;
      return true;
    }
